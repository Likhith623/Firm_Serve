...
export default function StaffAppointments() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch("/api/staff/appointments") // Use the staff-specific endpoint
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        console.log("Fetched staff appointments:", data);
            ...
            filteredAppointmentsList.map((appoin: appointments_table) => (
              ...
              >
                ...
                <div className="flex flex-col ml-2 mt-[-6px]">
                  ...
                  {appoin.Appointment_Client &&
                  appoin.Appointment_Client.length > 0 ? (
                    <div className="font-medium text-gray-700 mt-2">
                      Client(s):{" "}
                      {appoin.Appointment_Client.map(
                        (ac: any) => ac.Client?.name
                      ).join(", ")}
                    </div>
                  ) : (
